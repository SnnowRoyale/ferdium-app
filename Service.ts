...
/**
 * Retorna um caminho com um nome de arquivo único, se o arquivo já existir.
 * Se o arquivo não existir, retorna o próprio caminho.
 */
  ...
  initializeWebViewEvents({ handleIPCMessage, openWindow, stores }): void {
    ...
    if (webviewWebContents) {
      ...
      webviewWebContents.session.on('will-download', (event, item) => {
        ...
        // Se o usuário definiu uma pasta padrão para downloads, define um caminho único para o arquivo
        const downloadFolderPath =
          window['ferdium'].stores.settings.app.downloadFolderPath || '';
        if (downloadFolderPath !== '') {
          let filePath = join(downloadFolderPath, item.getFilename());
